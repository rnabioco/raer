...
- Replaced base R fisher test with c-wrapper to call htslib fisher test,
  which speeds up execution many fold.
...
- The `genomic-unstranded` option for the `library-type` argument in
  ...
  has been renamed to `unstranded`, and the `unstranded` option has been
  removed.
...
- added new function,
  ...
  to identify differentially editing sites in single cell data using
  fishers exact tests.
...
- support `BamFile` and `BamFileList` inputs to
  [`pileup_sites()`](https://rnabioco.github.io/raer/reference/pileup_sites.md)
  and
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md),
...
- rename `prep_for_de()` and `perform_de()` to
...
- default values for `edit_from` and `edit_to` for
...
- Added utility to screen scRNA-seq bam files for regions with oligo-dT
  mispriming
...
- added a strand bias stat `sor` using approach from GATK
  (StrandOddsRatio), and a confidence score
...
- Removed outdated or unused functionality:
  ...
  - bam tag indexing (`build_tag_index`, `show_tag_index`,
    `get_tag_bam`, )
...
- The bed indexing used in
  [`pileup_sites()`](https://rnabioco.github.io/raer/reference/pileup_sites.md)
  has been replaced with the region indexing approach from
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md).

- [`pileup_sites()`](https://rnabioco.github.io/raer/reference/pileup_sites.md)
  now requires a GRanges object rather than a bed file. The
  `bedfile`parameter has been removed and replaced with a `sites`
  parameter.

- Renamed `Ref` and `Var` output columns to `REF` and `ALT` and `nVar`
  was renamed to `nAlt`. This provides consistency with VCF format and
  consistency across
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md)
  and
  [`pileup_sites()`](https://rnabioco.github.io/raer/reference/pileup_sites.md)
...
- The `bam_flags` parameter used in `pileup_sites` and `pileup_cells`
  has been moved into the `FilterParam` class.
...
- The `bedindex` parameter for `pileup_sites` has been removed. This
  ...
  by the regional indexing used in
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md).
...
- A `RangedSummarizedExperiment` object is now directly returned from
  `pileup_sites`. Using `merge_pileups` is no longer necessary and is
  not an exported function.

- Renamed `get_pileup` to `pileup_sites` and `create_se` to
  `merge_pileups`
...
- Rewrote and renamed the single cell editing function `sc_editing` to
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md).
  [`pileup_cells()`](https://rnabioco.github.io/raer/reference/pileup_cells.md)
  does not require sorting and index by cell barcode, uses a new format
  to specify sites to query and requires providing the reference and
  alternate alleles of interest, writes to disk in a sparse matrix
  compatible format to reduce memory usage, and should have more
  performance as there is no need to query a fasta index.
...
- Added option to filter sites in pileup based on number of reads
  ...
  ([\#54](https://github.com/rnabioco/raer/issues/54))
