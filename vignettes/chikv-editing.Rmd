...
# n = 325352, so no effect, maybe increase threhold
...
tidy_var <- prep_tibble(mm_rse_filt, "Var") |>
  pivot_longer(-site) |> rename(var = value)
tidy_depth <- prep_tibble(mm_rse_filt, "depth") |>
  pivot_longer(-site) |> rename(depth = value)
tidy_edit_freq <- prep_tibble(mm_rse_filt, "edit_freq") |>
  pivot_longer(-site) |> rename(edit_freq = value)
...
# combine the above
tidy_all <- left_join(tidy_var, tidy_depth, by = c('site', 'name')) |>
...
```{r plot_counts}
# inspect the var counts
var_counts <- tidy_all |>
  group_by(treat, rep) |>
  dplyr::count(var)

ggplot(var_counts, aes(var, n, fill = rep)) +
  geom_col() +
  facet_grid(~treat) +
...
plot_tbl <- tidy_all |>
  ...
  dplyr::select(-(chrom:strand), -depth, -repName, -repFamily) |>
  pivot_longer(edit_freq:log_depth)

ggplot(plot_tbl, aes(var, value, fill = rep)) +
...
- `A>G` has the *lowest* editing frequencing. I'm not entirely sure how to think
...
```{r rep_families}
rep_counts <- tidy_all |>
  dplyr::select(treat, var, repName, repFamily) |>
  filter(!is.na(repName)) |>
  pivot_longer(-c(treat, var), names_to = 'repeat_class', values_to = 'rep_name') |>
  group_by(treat, var, repeat_class) |>
  count(rep_name)

filter(
  rep_counts,
  repeat_class == 'repFamily' & n >= 20, 
