...
N.B.: we also have unpublished data from an 8hr time point, and all have viral enrichement
data where coverage of virus goes up several fold.
...
ggplot(
  ...
  labs(
    title = 'Pileup summary of CHIKV 24h dLN scRNA-seq',
    x = '',
    y = '',
    fill = 'replicate'
  ) + 
...
hm <- ComplexHeatmap::Heatmap(
  heatmap_mtx,
  column_title = glue('Differentially edited A>G sites (n = {nc}; FDR = 0.05)'),
...
tidy_redi_tbl |>
  ...
  labs(
    x = '',
    y = '',
    title = glue('Annotation of {scales::comma(total_sites)} mouse A>G sites with REDIportal'),
    fill = 'Repeat class'
  ) +
  theme(
    legend.position='top', 
...
# qs::qsave(all_sce, fs::path(ext_path, '/sce/all_sce.qs'), nthreads = 10)
all_sce <- qs::qread(fs::path(ext_path, '/sce/all_sce.qs'), nthreads = 10)
```
...
tidy_sce <- function(x) {
  cli::cli_alert_info("tidying sce")
  sce_assays <- c('nVar', 'nRef')
  purrr::map(sce_assays, ~ tidy_sce_assay(x, .x)) |>
    purrr::reduce(left_join)
}

tidy_sce_assay <- function(sce, sce_assay) {
  cli::cli_alert_info("tidying assay: {sce_assay}")
 
  sce_sub <- sce[rowSums(assay(sce, sce_assay)) > 0, ]
  
  assay(sce_sub, sce_assay) |>
    as.data.frame() |>
    rownames_to_column("site") |>
    as_tibble() |>
    pivot_longer(-site, values_to = {{ sce_assay }}) |>
    filter( !! rlang::sym(sce_assay) > 0) |>
    separate_wider_delim(name, names = c('cb', 'rep'), delim = '_') |>
    separate_wider_delim(site, names = c('chrom', 'pos'), delim = '_', too_many = 'drop') |>
    janitor::clean_names()
}

tidy_sce_edits <-
  bind_rows(purrr::map(sce_outs, tidy_sce)) |>
  mutate(
    pos = as.integer(pos),
    n_ref = replace_na(n_ref, 0),
    n_var = replace_na(n_var, 0),
    depth = n_ref + n_var,
    edit_freq = n_var / depth
  ) |>
  # re-create colnames so they match the original all_sce
  unite(barcode, cb:rep)
```
...
# total sites
sc_edit_tbl <-
  left_join(tidy_sce_edits, tidy_sce_coldata, by = 'barcode') |>
  dplyr::select(orig.ident, cell_type, subtype) |>
  separate_wider_delim(orig.ident, names = c('treat', 'rep'), delim = '-') |>
  group_by(treat, cell_type, subtype)|>
  dplyr::count(sort = TRUE) |>
  mutate(
    treat = as_factor(treat), 
    treat = fct_relevel(treat, "mock", "CHIKV"),
  ...
  tidyr::complete(
    treat,
    nesting(cell_type, subtype),
    fill = list(n = 0)
  ) |>
  mutate(sites = 'all')

# significant sites
sig_de_sites <- filter(tidy_mm_tbl, !is.na(de_padj)) |>
  dplyr::select(chrom, pos) |>
  unique()

tidy_sce_edits_sig <- semi_join(tidy_sce_edits, sig_de_sites)

sc_edit_tbl_sig <-
  left_join(tidy_sce_edits_sig, tidy_sce_coldata, by = 'barcode') |>
  dplyr::select(orig.ident, cell_type, subtype) |>
  separate_wider_delim(orig.ident, names = c('treat', 'rep'), delim = '-') |>
  group_by(treat, cell_type, subtype)|>
  dplyr::count(sort = TRUE) |>
...
all_sc_edit_tbl <- bind_rows(sc_edit_tbl, sc_edit_tbl_sig)
```

### Viz subtype proportions
...
tidy_sce_coldata |>
  ...
  separate_wider_delim(orig.ident, names = c('treat', 'rep'), delim = '-') |>
  group_by(treat, rep, subtype) |>
  dplyr::count(sort = TRUE) |>
  group_by(treat, subtype) |>
  summarize(
    n_mean = mean(n),
    n_sd = sd(n)
  ) |>
  ...
  tidyr::complete(
    treat, subtype,
    fill = list(n_mean = 0, n_sd = 0)
  ) |>
  mutate(
    treat = fct_relevel(treat, c('mock', 'CHIKV'))
  )
...
ggplot(
  ...
  labs(
    ...
    y = '',
    fill = 'sample',
    title = 'Cells annotated in Mm / CHIKV scRNA-seq'
  ) +
  theme_minimal_vgrid() +
  theme(
    legend.position = 'top' 
...
### Plot editing levels per cell type

```{r plot_cell_types}
total_sites <- nrow(unique(dplyr::select(tidy_sce_edits, chrom, pos)))  
total_sites_sig <- nrow(unique(dplyr::select(tidy_sce_edits_sig, chrom, pos)))  

sites_labels <- c(
  all = glue('all A>G sites (n = {scales::comma(total_sites)})'),
  sig = glue('differentially edited A>G sites (n = {scales::comma(total_sites_sig)}, FDR = 0.05)')
)

# normalize site counts by cell type counts
norm_sc_edit_tbl <-
  left_join(all_sc_edit_tbl, subtype_cnts) |>
  mutate(n_norm = n / n_mean)

ggplot(
  norm_sc_edit_tbl,
  aes(
    x = fct_reorder(subtype, n),
    y = n_norm,
    fill = treat
  )
) +
  ...
  labs(
    x = '',
    y = 'Number of cells with 1 or more A>G sites',
    fill = 'sample',
    title = glue('RNA editing across cell types identifed in Mm / CHIKV scRNA-seq')
  ) +
  facet_grid(
    ~ sites,
    scales = "free_x",
...
# https://github.com/Wancen/airpart/blob/main/vignettes/airpart.Rmd
...
- report read length distribution to gauge library quality via TSO / pA trimming

### Filtering variants

- employ filter_mispriming filtter

- rewrite filter_multiallelic in Rcpp etc

### Single-cell

- Look for differential editing directly from the single-cell data instead of a external set (either between cell types in one virus / mock, or between virus mock). Pseudobulk with DESeq2 or Airpart.

- identify data sets in liver or brain that may enable identification of constitutive editing on APOE or GRIA2
