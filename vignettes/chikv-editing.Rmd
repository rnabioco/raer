...
ggplot(
  ...
  labs(
    title = 'Variants identified from pileup of dLN / CHIKV scRNA-seq data',
    x = '',
    y = '',
    fill = 'replicate'
  ) + 
...
plot_tbl <-
  tidy_all |>
    ...
    dplyr::select(
      -(chrom:strand), -depth, -starts_with('rep_') # , -rbpz, -vpb
      ) |>
...
- All variants have have apprxoimately the same `log_depth` and `rbpz`.

- `A>G` has one of hte lowest `vpb` values.

### Variants among repeat classes - OLD, see REDI below

```{r rep_families, eval = FALSE}
rep_counts <- tidy_all |>
  dplyr::select(treat, vnt, rep_name, rep_family) |>
  filter(!is.na(rep_name)) |>
  pivot_longer(-c(treat, vnt), names_to = 'rep_class', values_to = 'rep_name') |>
  group_by(treat, vnt, rep_class) |>
  dplyr::count(rep_name)

filter(
  rep_counts,
  rep_class == 'rep_family' & n >= 20,
  # examine the most abundant / interesting vntiants
  vnt %in% c('A>G', 'G>A', 'C>A', 'T>A')
) |>
...
rse_rows_ann <- VariantAnnotation::locateVariants(
  rowRanges(mm_rse_filt),
  txdb_common,
  AllVariants()
)

# TODO: figure out how to cross-ref GENEID in the result with the starting txdb
```
...
# qs::qsave(sig_de_res, fs::path(ext_path, "/rds/sig_de_res.rds"))
sig_de_res <- qs::qread(fs::path(ext_path, "/rds/sig_de_res.rds"))

ComplexHeatmap::Heatmap(
  assay(se_filt, "edit_freq")[head(sig_de_res, 50)$site, ],
  col = viridis::viridis(100),
  column_labels = se_filt$sample
)

sig_de_gr <- as_granges(sig_de_res)
genes_gr <- GenomicFeatures::transcripts(txdb)

# end up with multiple rows matching, so need to collapse somehow
# bioconductor makes my head hurt
plyranges::join_overlap_left(de_res_gr, genes_gr)

# TODO: join these sites with genes / gene regions for a more informative display
```

There are `nrow(sig_results)` significantly changed sites in the mouse transcriptome upon
virus infection.
