...
redi_plot_tbl <- 
  rowData(mm_rse_filt_redi) |>
    ...
    filter(!is.na(value)) |>
    # group_by(Ref, Var, name) |>
    group_by(name) |>
    dplyr::count(value, sort = TRUE) |>
    filter(str_detect(name, '^redi_'))

redi_plot_tbl |>
  filter(
    name == 'redi_func_gencode',
    str_detect(value, ',', negate = TRUE)
  ) |>
  ...
  labs(
    x = '',
    y = 'count',
    title = 'Sites in mm10 cross-referenced with REDIportal annotations'
  ) +
  theme(
