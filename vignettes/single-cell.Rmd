...
```{r}
bam_fn <- fns$bams
# positionally sort and index bam file
bf <- Rsamtools::sortBam(bam_fn, sub(".bam", "_sorted", bam_fn))
bfi <- Rsamtools::indexBam(bf)
...
Next we'll select editing sites to query. For this analysis we will use sites from the Rediportal database. To minimize processing time it can be useful to first identify sites with read coverage. `filter_by_coverage()` will exclude sites that do not have sufficient coverage. 
...
The `cell_barcodes` argument specifies which cell-barcodes will be quantified. This is typically set to the filtered set of barcodes corresponding to cell-containing droplets.
...
```{r}
e_sce <- e_sce[rowSums(assays(e_sce)$nVar > 0) >= 5, ]
e_sce <- calc_edit_frequency(e_sce, replace_na = FALSE)
...
```{r}
to_plot <- rownames(altExp(sce))[order(rowSums(assay(altExp(sce), "nVar")), decreasing = TRUE)]

lapply(to_plot[1:10], function(x) {
  plotUMAP(sce, colour_by = x, by_exprs_values = "nVar")
})
```
