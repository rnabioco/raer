...
Here we demonstrate how to use the *raer* package to a) quantify known RNA editing in droplet scRNA-seq dataset,  b) identify editing sites with condition specific editing in bulk RNA-seq data, and c) predict novel editing sites from bulk RNA-seq. Lastly we discuss applying the functionality provided by the *raer* to characterize sequencing libraries that use mismatch frequencies as diagnostic signals. 
...
To exclude duplicate reads derived from PCR, `pileup_cells()` can use a UMI sequence, supplied via the `umi_tag` argument, to only count 1 read for each CB-UMI pair at each position. 
...
params <- FilterParam(min_mapq = 255L, # required read MAPQ score
...
- `find_celltype_de()` will perform statistical testing to identify sites with different editing frequencies between clusters/cell types.  
...
Lastly, we will again use the database of known human editing sites from REDIPortal. 
...
Internally, `pileup_sites()` will generate base count pileups in memory. However these pileups can optionally be stored in tabix-indexed files, for later retrieval, if needed.
...
fp <- FilterParam(
    ...
    trim_3p = 5,
    min_base_quality = 30L,           # minimum base quality score
    min_mapq = 255L,                  # minimum MAPQ read score
...
With the above filtering approach we obtain a set of putative editing sites. The specificity of the filtering can be estimated by examining the number of A-to-G changes relative to other variants. A-to-I RNA editing is far more common than other types of editing (e.g. C->U editing by APOBEC enzymes) in human datasets so the majority of the variants should by A-to-G. In this vignette data all of the identified sites are A-to-G.

```{r}
table(paste0(rowData(rse)$REF, rowData(rse)$ALT))
```
...
## Characterizing other types of mismatch signals

The pileup routines in the raer pacakge are general and can be used to  quantify other types of mismatch signals, such as C-to-U APOBEC mediated RNA editing, or mismatch signals introduced by nucleoside analogs, such as 4SU labeling (e.g. SLAM-seq).
