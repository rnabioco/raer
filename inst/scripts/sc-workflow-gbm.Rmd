---
title: "Glioblastoma single cell RNA-editing workflow"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r}
if(!file.exists(file.path(data_dir, "gbm_scdata", "gbm.rds"))){
  ...
  # check that all cellbarcodes are in bam file
  bcs <- read.table(file.path(data_dir, "cbs.txt"))$V2
  stopifnot(colnames(sce) %in% bcs)
  saveRDS(sce, file.path(data_dir, "gbm_scdata", "gbm.rds"))
}
sce <- readRDS(file.path(data_dir, "gbm_scdata", "gbm.rds"))
...
```{r}
if(!file.exists(file.path(data_dir, "gbm_scdata", "gbm_pileups.rds"))){
  bamfn <- file.path(data_dir, "Parent_SC3v3_Human_Glioblastoma_possorted_genome_bam_subsampled_cb_sorted.bam")
  bedfn <- file.path(data_dir, "rediportal_hg38.bed")
  # ~ 10 sec.
  build_tag_index(bamfn)
  dir.create(file.path(data_dir, "cluster-bams"), showWarnings = FALSE)
  ...
  #started at 10:23
  #finished at 10:33
  saveRDS(res, file.path(data_dir, "gbm_scdata", "gbm_pileups.rds"))
}

res <- readRDS(file.path(data_dir, "gbm_scdata", "gbm_pileups.rds"))
...
```{r}
# ~ 2k sites remaining, make a heatmap
