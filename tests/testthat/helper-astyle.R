...
astyle <- function(extra_args = character()) {
  ...
  src_path <- normalizePath(vcapply(c("../../src"), testthat::test_path))
  src_files <- dir(src_path, "[.](?:cpp|h|c)$", recursive = TRUE, full.names = TRUE)
  astyle_files <- grep("(?:src/ext|init.c)", src_files, value = TRUE, invert = TRUE)
