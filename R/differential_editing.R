...
calc_edit_frequency <- function(
  ...
  if (
    is(assay(rse, to_col), "sparseMatrix") ||
...
count_edits <- function(
  ...
  n_pass_filter <- Matrix::colSums(
    (assay(se, "edit_freq") > edit_frequency) &
...
make_de_object <- function(
  ...
  if (!"edit_freq" %in% assayNames(rse)) {
    cli::cli_abort(c(
      "`edit_freq` not present in assay",
...
run_deseq2 <- function(
  ...
  alt_treatment <- colMeans(mod_mat[
    dds$condition == condition_treatment &
      dds$count == "alt",
  ])
  ref_treatment <- colMeans(mod_mat[
    dds$condition == condition_treatment &
      dds$count == "ref",
  ])

  alt_control <- colMeans(mod_mat[
    dds$condition == condition_control &
      dds$count == "alt",
  ])
  ref_control <- colMeans(mod_mat[
    dds$condition == condition_control &
      dds$count == "ref",
  ])

  # This finds editing specific to the treatment condition
  treatment_vs_control <- DESeq2::results(
    dds,
...
run_edger <- function(
  ...
  # Pull out the model matrix for all comparisons of interest
  alt_treatment <- colMeans(design[
    deobj$condition == condition_treatment &
      deobj$count == "alt",
  ])
  ref_treatment <- colMeans(design[
    deobj$condition == condition_treatment &
      deobj$count == "ref",
  ])

  alt_control <- colMeans(design[
    deobj$condition == condition_control &
      deobj$count == "alt",
  ])
  ref_control <- colMeans(design[
    deobj$condition == condition_control &
      deobj$count == "ref",
  ])

  treatment_vs_control <- edgeR::glmLRT(
    fit,
    contrast = (alt_treatment - ref_treatment) -
      (alt_control - ref_control)
  )

  treatment_vs_control <- edgeR::topTags(
...
find_scde_sites <- function(
  ...
  depth_summary <- scran::summaryMarkerStats(
    sce,
    sce[[group]],
    assay.type = "depth",
    BPPARAM = BPPARAM
